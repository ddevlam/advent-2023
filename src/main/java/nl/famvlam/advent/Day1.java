...
public class Day1 {
    ...
    public static class LineReader {
        ...
        public LineReader(final String lines){

            this.lines = lines;
        }

        int getCalibrationPartOne(){
            return lines.lines()
                    .map(Calibration::new)
                    .mapToInt(Calibration::valuePartOne)
                    .sum();
        }

        int getCalibrationPartTwo(){
            return lines.lines()
                    .map(Calibration::new)
                    .mapToInt(Calibration::valuePartTwo)
                    .sum();
        }
    }

    static List<Conversion> conversions = List.of(
            new Conversion("one", "o1e"),
            new Conversion("two", "t2o"),
            new Conversion("three", "t3e"),
            new Conversion("four", "f4r"),
            new Conversion("five", "f5e"),
            new Conversion("six", "s6x"),
            new Conversion("seven", "s7n"),
            new Conversion("eight", "e8t"),
            new Conversion("nine", "n9e"),
            new Conversion("zero", "z0o")
    );

    record Conversion(String from, String to) {

        String convert(String line){
            return line.toLowerCase().replaceAll(from, to);
        }
    }

    record Calibration(String line) {
        ...
        int valuePartTwo(){
            ...
            for(int i = 0; i < conversions.size(); i++){
                converted = conversions.get(i).convert(converted);
            }

            return toDigit(converted);
        }

        int toDigit(String toConvert){

            final var onlyDigits = toConvert.replaceAll("\\D", "");
            final var combined = onlyDigits.charAt(0) + "" + onlyDigits.charAt(onlyDigits.length()-1);
