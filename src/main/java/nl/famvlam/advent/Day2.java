...
var lines = """
        ...
        System.out.println("Part one : " + Day2.BagChecker.partOne(lines, constraints));
        System.out.println("Part two : " + Day2.BagChecker.partTwo(lines));
    }

    static class BagChecker {

        static Integer partOne(final String input, final EnumMap<Color, Integer> constraints){
            return new Day2.LineReader().parseInput(input)
                    .stream()
                    .filter(game -> game.wouldFit(constraints))
                    .mapToInt(Game::id)
                    .sum();
        }

        static Integer partTwo(final String input){
            return new Day2.LineReader().parseInput(input)
                    .stream()
                    .mapToInt(Game::determinePower)
                    .sum();
        }
    ...
    static class LineReader {
        ...
        public List<Game> parseInput(final String input){
            return input.lines()
                    .map(line -> line.split(":"))
                    .map(Game::fromGameInfo)
                    .toList();
        }

    }

    record Game(int id, List<Cubes> cubeInfo){

        static Game fromGameInfo(String[] gameInfo){
            var gameId = gameInfo[0].substring("Game ".length());
            var gameCubeInfo = gameInfo[1];

            return new Game(Integer.parseInt(gameId), Cubes.fromGameCubeInfo(gameCubeInfo));
        }

        boolean wouldFit(EnumMap<Color, Integer> input){
            return input.keySet()
                    .stream()
                    .allMatch(color -> cubeInfo
                            .stream()
                                    .allMatch(cubes -> {
                                        var byColor = cubes.byColor.get(color);
                                        return byColor == null || byColor <= input.get(color);
                                    }));
        }

        int determinePower(){
            return smallestNrNeeded().values()
                    .stream()
                    .reduce(1, (pre, nex) -> pre * nex);
        }

        EnumMap<Color, Integer> smallestNrNeeded(){
            var smallest = new EnumMap<Color, Integer>(Color.class);
            cubeInfo.forEach(cubes -> {
                cubes.byColor.forEach((color, total) ->{
                    var currentValue= smallest.getOrDefault(color, 0);
                    var newValue = total > currentValue ? total : currentValue;
                    smallest.put(color, newValue);
                });
            });
            return smallest;
        }
    ...
    record Cubes(EnumMap<Color, Integer> byColor){

        static List<Cubes> fromGameCubeInfo(String gameCubeInfo){
            var result = new ArrayList<Cubes>();
            var set = gameCubeInfo.split(";");
            for(int i = 0; i < set.length; i++){
                var cubes = set[i].split(",");
                var map = new EnumMap<Color, Integer>(Color.class);
                for(int j = 0; j < cubes.length; j++){
                    var cubeInfo = cubes[j].trim().split(" ");
                    var cubeTotal = cubeInfo[0];
                    var cubeColor = Color.valueOf(cubeInfo[1].toUpperCase());

                    map.put(cubeColor, map.getOrDefault(cubeColor, 0) + Integer.parseInt(cubeTotal));
                }
                result.add(new Cubes(map));
            }
